...
/// Tiger S-box tables (t1, t2, t3, t4 are at offsets 0, 256, 512, 768)
#[rustfmt::skip]
...
fn round(a: &mut u64, b: &mut u64, c: &mut u64, x: u64, mul: u64) {
    ...
    *a = a.wrapping_sub(
        t1((*c & 0xFF) as usize)
            ^ t2(((*c >> 16) & 0xFF) as usize)
            ^ t3(((*c >> 32) & 0xFF) as usize)
            ^ t4(((*c >> 48) & 0xFF) as usize),
    );
    // b += t4[c[1]] ^ t3[c[3]] ^ t2[c[5]] ^ t1[c[7]]
    *b = b.wrapping_add(
        t4(((*c >> 8) & 0xFF) as usize)
            ^ t3(((*c >> 24) & 0xFF) as usize)
            ^ t2(((*c >> 40) & 0xFF) as usize)
            ^ t1(((*c >> 56) & 0xFF) as usize),
    );
    *b = b.wrapping_mul(mul);
}

/// Pass through 8 rounds
fn pass(a: &mut u64, b: &mut u64, c: &mut u64, x: &[u64; 8], mul: u64) {
    round(a, b, c, x[0], mul);
    round(b, c, a, x[1], mul);
    round(c, a, b, x[2], mul);
    round(a, b, c, x[3], mul);
...
/// Tiger compress function (configurable passes)
fn tiger_compress(state: &mut [u64; 3], block: &[u8; 64], extra_passes: u32) {
    let mut x = [0u64; 8];
    for i in 0..8 {
        x[i] = u64::from_le_bytes(block[i * 8..(i + 1) * 8].try_into().unwrap());
    }

    let (aa, bb, cc) = (state[0], state[1], state[2]);
    let (mut a, mut b, mut c) = (aa, bb, cc);
    ...
    // Additional passes for Tiger-4 and beyond
    // After each extra pass, PHP does value rotation: tmpa=a; a=c; c=b; b=tmpa;
    for _ in 0..extra_passes {
        key_schedule(&mut x);
        pass(&mut a, &mut b, &mut c, &x, 9);
        // Rotate values: a=c, c=b, b=a (in PHP order)
        let tmp = a;
        a = c;
        c = b;
        b = tmp;
...
/// Context for Tiger hash computation
struct TigerContext {
    state: [u64; 3],
    buffer: [u8; 64],
    count: u64,
    extra_passes: u32, // 0 for Tiger-3, 1 for Tiger-4
}

impl TigerContext {
    fn new(extra_passes: u32) -> Self {
        Self {
    ...
    fn update(&mut self, data: &[u8]) {
        let mut offset = 0;
        let buf_pos = (self.count & 63) as usize;

        self.count += data.len() as u64;

        // Fill buffer if we have partial data
        if buf_pos > 0 {
            let copy_len = (64 - buf_pos).min(data.len());
            self.buffer[buf_pos..buf_pos + copy_len].copy_from_slice(&data[..copy_len]);
            if buf_pos + copy_len < 64 {
                return;
            }
            tiger_compress(&mut self.state, &self.buffer, self.extra_passes);
            offset = copy_len;
        }
        ...
        while offset + 64 <= data.len() {
            ...
            tiger_compress(&mut self.state, &block, self.extra_passes);
            offset += 64;
        }
    ...
    fn finalize(mut self) -> [u8; 24] {
        let buf_pos = (self.count & 63) as usize;
        let bit_count = self.count << 3;

        // Pad with 0x01
        self.buffer[buf_pos] = 0x01;
        for i in buf_pos + 1..64 {
            self.buffer[i] = 0;
        }
...
/// Compute Tiger-192 hash with 3 passes
pub fn tiger192_3(data: &[u8]) -> [u8; 24] {
    let mut ctx = TigerContext::new(0); // 0 extra passes = 3 total
    ctx.update(data);
    ctx.finalize()
}

/// Compute Tiger-192 hash with 4 passes
pub fn tiger192_4(data: &[u8]) -> [u8; 24] {
    let mut ctx = TigerContext::new(1); // 1 extra pass = 4 total
    ctx.update(data);
    ctx.finalize()
}
