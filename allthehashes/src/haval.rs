...
mod tests {
    ...
    // Most of these were taken from PHP 8.x with
    //   php -r 'echo hash("havalXXX,Y", "...");'
    // but that one-liner cannot have produced the three 100003-byte vectors below:
    // their input is SHA256(0u64_le) || SHA256(1u64_le) || ... truncated to 100003
    // bytes, which is not expressible as a PHP string literal. Those were generated by
    // building the input and piping it to PHP, and the comment used to claim otherwise.
    //
    // None of that was reproducible from this repository, so agreement with PHP and
    // agreement with whatever was pasted in here were indistinguishable. The oracle in
    // tests/php_vectors.rs is the actual check now: it holds 450 PHP-generated HAVAL
    // vectors, thirty lengths for each of the fifteen variants, chosen to bracket the
    // 128-byte block and the padding boundaries, and tests/generate_php_vectors.php
    // regenerates them. Treat the values below as the fast in-file smoke test and that
    // as the provenance.
