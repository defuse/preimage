...
impl IndexEntry {
    ...
    pub fn read_bulk(reader: &mut impl Read, buf: &mut [Self], count: usize) -> std::io::Result<()> {
        assert!(count <= buf.len(), "count {count} exceeds buffer length {}", buf.len());
        if count == 0 {
            return Ok(());
        }
...
mod tests {
    ...
    fn test_position_roundtrip() {
        ...
        for &pos in positions {
            ...
            assert_eq!(entry.position(), pos, "position roundtrip failed for {pos:#x}");
        }
    }
